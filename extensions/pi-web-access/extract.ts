...
function isUnsupportedMediaUrl(url: string): boolean {
  try {
    const path = new URL(url).pathname.toLowerCase();
    return /\.(mp4|mov|webm|avi|mkv|mpg|mpeg|wmv|flv|3gp|m4v)$/i.test(path)
      || /(?:^|\.)youtu\.be$/i.test(new URL(url).hostname)
      || /(?:^|\.)youtube\.com$/i.test(new URL(url).hostname);
  } catch {
    return false;
  }
}
...
export async function extractContent(url: string, signal?: AbortSignal, options?: ExtractOptions): Promise<ExtractedContent> {
  ...
  if (isUnsupportedMediaUrl(url)) {
    return {
      url,
      title: "",
      content: "",
      error: "YouTube and local video analysis are not included in this sz-pi-extensions web access build."
    };
  }
  ...
  try {
    ...
    if (isConfigParseError(err)) return { url, title: "", content: "", error: message };
  }
  ...
  const guidance = [
    ...
    "  • Try a more specific URL or a text/PDF source",
  ].join("\n");
